...
Returns the natural logarithm of the [probability mass function][pmf] (PMF) for a [discrete uniform][discrete-uniform-distribution] distribution with parameters `a` (minimum support) and `b` (maximum support).
...
-   **a**: `[in] int32_t` location parameter.
-   **b**: `[in] int32_t` scale parameter.

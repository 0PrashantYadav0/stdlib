...
```c
double out = stdlib_base_dists_binomial_mean( 10, 0.1 );
// returns ~1.0
```
...
```c
double stdlib_base_dists_binomial_mean( const int n, const double p );
```
...
int main( void ) {
    ...
    for ( i = 0; i < 25; i++ ) {
        ...
        p = ( (double)rand() / (double)RAND_MAX ); // random probability between 0 and 1

...
Returns the [cumulative distribution function][cdf] for a [binomial][binomial-distribution] distribution with number of trials `n` and success probability `p`.

```c
double out = stdlib_base_dists_binomial_cdf( 3.0, 20, 0.2 );
// returns ~0.411
```
...
int main( void ) {
    ...
    for ( i = 0; i < 25; i++ ) {
        ...
        x = stdlib_base_ceil( random_uniform( 0.0, 100.0 ) );
        y = stdlib_base_dists_binomial_cdf( x, n, p );
        printf( "x: %d, n: %d, p: %lf, F(x;n,p): %lf\n", x, n, p, y );
    }

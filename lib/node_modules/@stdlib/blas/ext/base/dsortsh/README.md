...
stdlib_strided_dsortsh( 2, -1, x, 1 );
```
...
```c
stdlib_strided_dsortsh( const CBLAS_INT N, const double order, double *X, CBLAS_INT strideX );
```
...
stdlib_strided_dsortsh_ndarray( 4, 1, x, 1, 0 );
```
...
```c
stdlib_strided_dsortsh_ndarray( const CBLAS_INT N, const double order, double *X, CBLAS_INT strideX, CBLAS_INT offsetX );
```
